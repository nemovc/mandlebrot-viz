...
export function samplePalette(palette: ColorConfig['palette'], t: number): [number, number, number] {
	const stops = palette;
	if (stops.length === 0) return [0, 0, 0];
	if (t <= stops[0].stop) return hexToRgb(stops[0].color);
	if (t >= stops[stops.length - 1].stop) return hexToRgb(stops[stops.length - 1].color);
