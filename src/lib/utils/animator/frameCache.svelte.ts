...
function createFrameCache() {
  const frames = new SvelteMap<number, ImageBitmap>();
  ...
  const isReady = $derived.by(() => {
    ...
    return Array.from(
      ...
      (_, k) => (animationState.currentFrame + k) % animationState.project.totalFrames
  ...
  async function startFrom(startFrame: number, project: AnimationProject, priority = 2) {
    cancelBuild();
    ...
    async function lane() {
      while (!signal.aborted && idx < order.length) {
        ...
        const bm = await renderFrameLowRes(frame, snap, lw, lh, signal, priority, ++_seq);
