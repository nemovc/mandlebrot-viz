<script lang="ts">
	...
	const resolutions = [
		{ label: `Current view (${physW}×${physH})`, w: physW, h: physH, wide: true },
	...
	function findLocationName(cx: string, cy: string, zoom: number): string | null {
		...
		const match = (loc: Location) =>
			loc.zoom === zoom && Math.abs(re - loc.re) < eps && Math.abs(im - loc.im) < eps;
		return PRESET_LOCATIONS.find(match)?.name ?? savedLocations.all.find(match)?.name ?? null;
	}
	...
	function drawOverlay(
		...
		const algorithm = ALGORITHMS.find((a) => (a.value = colorConfig.algorithm))?.label ?? 'Custom';
	...
	async function exportPng() {
		...
		try {
			...
			if (showOverlay) drawOverlay(ctx, w, h, cx, cy, zoom, maxIter, power, colorConfig);
	...
	const modalActions = $derived<ModalAction[]>(
		phase === 'done'
			? [
					{ label: 'Export Another', callback: reset },
					{
						label: `Save ${formats[selectedFormat].label}`,
						color: 'blue',
						callback: saveFile,
						isDefault: true
					}
			...
			: [
					{ label: 'Cancel', callback: onclose },
					{
						label: `Export ${formats[selectedFormat].label}`,
						color: 'blue',
						callback: exportPng,
						isDefault: true
					}
			...
			<div class="flex flex-col gap-2">
				...
				{:else}
					<p class="text-xs text-neutral-400">
						Centered on the current view, but the field of view will differ due to the different
			...
			{#if phase === 'exporting'}
				<div class="flex flex-col gap-2">
					...
					<div class="flex items-center justify-between text-xs text-neutral-400">
						...
						<button class="text-red-400 hover:text-red-300" onclick={() => cancelFn?.()}
							>Cancel</button
						>
					</div>
				</div>
