<script lang="ts">
	...
	let { name, pool, color = 'bg-blue-400' }: {
		name: string;
...
<div class="flex flex-col gap-1">
	<div class="font-mono text-xs text-neutral-300 leading-4">
		{abbr} <span class="text-neutral-600">(s{poolSize})</span>
		<span class="ml-1"><span class="text-neutral-600">i</span>{idle}</span>
		<span class="ml-1"><span class="text-neutral-600">a</span>{active}</span>
		<span class="ml-1"><span class="text-neutral-600">q</span>{queued}</span>
	</div>
	<div class="flex items-center gap-2">
		<div class="flex-1 h-1 bg-neutral-700 rounded overflow-hidden">
			<div class="h-full {color} transition-all duration-150" style="width:{pct}%"></div>
		</div>
