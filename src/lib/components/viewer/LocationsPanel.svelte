<script lang="ts">
  ...
  function isAt(loc: Location): boolean {
    ...
    const eps = scaleForZoom(loc.zoom) * 2; // ~2 pixels at that zoom level
    return (
  ...
  const wouldOverwrite = $derived(savedLocations.exists(saveName.trim()));
  ...
  function confirmSave() {
    ...
    if (wouldOverwrite && !overwriteConfirmed) { overwriteConfirmed = true; return; }
    savedLocations.save({
      name: saveName.trim(),
  ...
  <div class="overflow-y-auto max-h-[50vh] p-2 flex flex-col gap-0">
    {#if viewerState.power !== 2}
      <p class="text-yellow-500 text-xs mb-2 leading-snug">
        Preset locations are for exponent 2
      </p>
    {/if}
    ...
    {#if saving}
      <div class="flex flex-col gap-1">
        ...
        {#if overwriteConfirmed}
          <p class="text-yellow-500 text-xs">Overwrite "{saveName.trim()}"?</p>
        ...
        <div class="flex gap-1">
          ...
          <button
            ...
            onclick={confirmSave}>{overwriteConfirmed ? 'Confirm' : 'Save'}</button
          >
        </div>
      </div>
    {:else}
